...
try {
  ...
  // Silent validation - remove Firebase project warnings
} catch (error) {
...
try {
  ...
  setPersistence(auth, inMemoryPersistence).catch((error) => {
    ...
    setPersistence(auth, browserSessionPersistence).catch((fallbackError) => {
      // Silent fallback - don't log warnings for persistence issues
    });
  });
...
try {
  ...
  // Silent fallback for analytics - not critical
  analytics = null;
}
...
export const handleFirebaseError = (error) => {
  ...
  switch (error.code) {
    ...
    case 'auth/invalid-credential':
      return 'User not found. Please check your credentials or register first.';
